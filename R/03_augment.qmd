...
For further analyses, it's more convenient to work in a long format: one row per (gene × sample) measurement.
...
pivot_longer(
cols = -c(Ensembl_ID, sum_gene_exp),
names_to = "sample",
values_to = "expression"
)

gen_exp_long |> slice_sample(n = 10)
```
...
biological measurements (gene-exp) + patient characteristics (phenotype).
...
NA_character_ ensures that missing age values remain coded as valid character-type missing values in the age category variable.
