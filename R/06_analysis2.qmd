...
In this document, I explore the gene expression dataset using Principal Component Analysis (PCA).

I work with the augmented tidy dataset created in 03_augment.qmd and check whether patients separate in the PCA space depending on clinical variables such as the alive status and age category.
...
Since PCA works best with genes that vary a lot across samples, I select the top 500 most variable genes based on the variance of log2_expression.
...
Here, I create a wide matrix with one row per sample (patient) and one column per gene (for the selected 500 genes). I also extract the clinical variables so I can link them to the PCA results later.
...
I extract the numeric expression data, centre and scale it, and run PCA using prcomp().
...
```{r}
ggsave(filename = "../results/plots/06_pca_scree.png",
  plot = scree_plot,
  width = 7,
  height = 5)
```
...
Here, I plot PC1 vs PC2 and colour the samples by alive status and shape by age category.
...
```{r}
ggsave(filename = "../results/plots/06_pca_pc1_pc2.png",
  plot = pc1_pc2_plot,
  width = 7,
  height = 5)
```
