...
- `log2_expression` (numeric, gene expression on logarithmic scale)
- `age_cat` (chr, age category the patient belongs to: young, middle, senior)
...
The following graphs and statistics will allow to visualize the content of the data.
...
full_data_numeric_summary <- full_data |> 
...
```
The graph shows how age at diagnosis varies across different tumor stages.
Stage IIB, IIC, IIIC, and IV show wide spreads in age, meaning patient ages vary a lot within those groups. As seen in the table above, there are few data points for stages IC, IIA, IIB and IIIA. There are some outliers, especially in Stage IIIB. 
...
The mean log2 gene expression level is at around 1.8. 

```{r}

high_genes <- full_data$log2_expression[full_data$log2_expression > 3.5]
