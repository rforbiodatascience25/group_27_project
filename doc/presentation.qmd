...
<<<<<<< HEAD
The histogram displays the distribution of log2-transformed gene-expression values.
=======
-   Histogram of log2-transformed gene-expression values

-   Genes with zero or extremely low expression removed
>>>>>>> 2a195a7d10e33bc5029c39b186ac558c4ee74dc9

-   Mean expression shifted upward; distribution more symmetric

<<<<<<< HEAD
![](plots/04_age_tumorstage.png)
=======
-   Confirms dataset is clean and suitable for analysis
...
![](plots/05_figo_stages.png){width="70%"}

![](plots/05_immune_ethnicity.png){width="85%"}
:::
:::::
...
::: column
<<<<<<< HEAD
![](plots/06_pca_scree.png){width="80%"} ![](plots/06_pca_pc1_pc2.png){width="80%"}
=======
...
::: column
<<<<<<< HEAD
![](plots/07_volcano_alive_vs_dead.png){width="80%"} ![](plots/07_boxplots_alive_vs_dead.png){width="80%"}
=======
...
1.  [*Clinical variables distributions*]{style="color:#f8a47d;"}: Exploration of FIGO stage, age, ethnicity and vital status show highly imbalanced distribution

    → Difficult to detect strong association

2.  [*PCA of gene expression*]{style="color:#f8a47d;"} : Showed that the major sources of variation in transcriptomic data were not related to other clinical variables (no distinct clusters)

    → Transcriptomic differences betweens groups are subtle

3.  [*Differential expression*]{style="color:#f8a47d;"}: Out of 14'000 tested genes, only two genes remained significant after FDR correction

    → Survival is not a major driver expression differences in this cohort

[**Conclusion**]{style="color:#f8a47d;"}

The analyses show that clinical variables alone cannot explain expression variability, and demonstrate that the dataset likely reflects complex biology, where survival differences cannot be captured by single-gene effects.
